...
# Los endpoints de PRs devuelven un ARRAY, pero ante cualquier fallo (rate
# limit, token sin permiso, endpoint caido) contestan un OBJETO
# {"message": ...}. Recorrer eso con `.[]` hace que jq intente indexar strings
# -"Cannot index string with string \"merged_at\""- y salga con codigo 5; con
# `set -e` eso tumbaba el step de notificacion y pintaba el workflow en rojo
# incluso cuando el deploy habia terminado bien. Un aviso de WhatsApp no puede
# decidir el resultado de un deploy: si la respuesta no es un array se anota el
# motivo y se sigue con lista vacia.
pulls_json() { # $1 = URL del endpoint de PRs
  local cuerpo tipo
  cuerpo="$(gh_api "$1" || true)"
  tipo="$(printf '%s' "$cuerpo" | jq -r 'type' 2>/dev/null || echo desconocido)"
  if [ "$tipo" = "array" ]; then
    printf '%s' "$cuerpo"
  else
